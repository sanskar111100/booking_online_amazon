...
if __name__ == '__main__':
    ...
    parentCardNo = '4718650900029049'
    parentCardPass = 'Oswal@1234'
    ...
    xcount = 0
    typeRep('//*[@name="textlogin"]', parentCardNo)
    typeRep('//*[@name="textpassword"]', parentCardPass)
    while 1:
        try:
    ...
    creditCardCsv = pd.DataFrame(columns = ['cardName', 'cardNumber', 'expMonth', 'expYear', 'cvv', 'staticPass', 'parentCard'])
    ...
    while count<noOfCards:
        ...
        #request time
            ...
            while 1:
                try: 
                    ...
                    curTime_2 = datetime.datetime.now()
                    if (curTime_2 - curTime).seconds>7:
                        flag = 0
                        clickRep(b1, '//*[@id="cmdCancel"]')
                        break
                    continue
            if flag==1:
                break
        ...
        creditCardCsv.loc[len(creditCardCsv.index)] = [cardName, cardNumber, expMonth, expYear, cardCvv, staticPass, parentCardNo] 
        count += 1
        #time.sleep(30)
